  ...
  <div>
    ...
    <div style="margin: 8px 0 16px; display: flex; align-items: center; gap: 8px">
      ...
      <NuxtLink
        ...
        :style="{ background: $i18n.locale === 'zh' ? '#409EFF' : '#fff', color: $i18n.locale === 'zh' ? '#fff' : '#333' }"
      >
        中文
      </NuxtLink>
      <NuxtLink
        ...
        style="padding: 4px 14px; border: 1px solid #ccc; border-radius: 4px; text-decoration: none"
        :style="{ background: $i18n.locale === 'en' ? '#409EFF' : '#fff', color: $i18n.locale === 'en' ? '#fff' : '#333' }"
      >
        EN
      </NuxtLink>
    </div>
    ...
    <template v-else>
      ...
      <div class="demo-card">
        <div class="demo-card__header">
          ...
          <span style="color: #67C23A; font-size: 12px; margin-left: 8px">⬤ REST + lazy: false</span>
        </div>
        <p class="demo-card__desc">端点: <code>/api/dict/list</code> — gender / status / industry</p>
        <table class="demo-table">
          <thead><tr><th>type</th><th>value</th><th>translate</th></tr></thead>
          <tbody>
            <tr v-for="item in d1Gender" :key="'d1g-' + item.value">
              <td>gender</td><td>{{ item.value }}</td><td>{{ d1GenderT(item.value) }}</td>
            </tr>
            <tr v-for="item in d1Status" :key="'d1s-' + item.value">
              <td>status</td><td>{{ item.value }}</td><td>{{ d1StatusT(item.value) }}</td>
            </tr>
            <tr v-for="item in d1Industry" :key="'d1i-' + item.value">
              <td>industry</td><td>{{ item.value }}</td><td>{{ d1IndustryT(item.value) }}</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div class="demo-card">
        ...
        <p class="demo-card__desc">端点: <code>/api/dict/list2</code> — gender(【源2】) / status(【源2】) / priority</p>
        <table class="demo-table">
          <thead><tr><th>type</th><th>value</th><th>translate</th></tr></thead>
          <tbody>
            <tr v-for="item in d2Gender" :key="'d2g-' + item.value">
              <td>gender</td><td>{{ item.value }}</td><td>{{ d2GenderT(item.value) }}</td>
            </tr>
            <tr v-for="item in d2Status" :key="'d2s-' + item.value">
              <td>status</td><td>{{ item.value }}</td><td>{{ d2StatusT(item.value) }}</td>
            </tr>
            <tr v-for="item in d2Priority" :key="'d2p-' + item.value">
              <td>priority</td><td>{{ item.value }}</td><td>{{ d2PriorityT(item.value) }}</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div class="demo-card">
        ...
        <p class="demo-card__desc">端点: <code>/api/dict/list2</code>（同 dicts2 数据，惰性版本检查）</p>
        <table class="demo-table">
          <thead><tr><th>type</th><th>value</th><th>translate</th></tr></thead>
          <tbody>
            <tr v-for="item in d3Gender" :key="'d3g-' + item.value">
              <td>gender</td><td>{{ item.value }}</td><td>{{ d3GenderT(item.value) }}</td>
            </tr>
            <tr v-for="item in d3Status" :key="'d3s-' + item.value">
              <td>status</td><td>{{ item.value }}</td><td>{{ d3StatusT(item.value) }}</td>
            </tr>
            <tr v-for="item in d3Priority" :key="'d3p-' + item.value">
              <td>priority</td><td>{{ item.value }}</td><td>{{ d3PriorityT(item.value) }}</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div class="demo-card">
        <div class="demo-card__header">
          ...
          <span style="color: #67C23A; font-size: 12px; margin-left: 8px">⬤ adapter + lazy: false</span>
        </div>
        <p class="demo-card__desc">适配器: <code>~/dict/dicts4-adapter.ts</code> — gender(【源4】) / paymentStatus / orderType</p>
        <table class="demo-table">
          <thead><tr><th>type</th><th>value</th><th>translate</th></tr></thead>
          <tbody>
            <tr v-for="item in d4Gender" :key="'d4g-' + item.value">
              <td>gender</td><td>{{ item.value }}</td><td>{{ d4GenderT(item.value) }}</td>
            </tr>
            <tr v-for="item in d4Payment" :key="'d4p-' + item.value">
              <td>paymentStatus</td><td>{{ item.value }}</td><td>{{ d4PaymentT(item.value) }}</td>
            </tr>
            <tr v-for="item in d4Order" :key="'d4o-' + item.value">
              <td>orderType</td><td>{{ item.value }}</td><td>{{ d4OrderT(item.value) }}</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div class="demo-card">
        <div class="demo-card__header">
          ...
          <span style="color: #E6A23C; font-size: 12px; margin-left: 8px">⬤ adapter + lazy: true</span>
        </div>
        <p class="demo-card__desc">适配器: <code>~/dict/dicts5-adapter.ts</code> — currency / logistics / warehouse</p>
        <table class="demo-table">
          <thead><tr><th>type</th><th>value</th><th>translate</th></tr></thead>
          <tbody>
            <tr v-for="item in d5Currency" :key="'d5c-' + item.value">
              <td>currency</td><td>{{ item.value }}</td><td>{{ d5CurrencyT(item.value) }}</td>
            </tr>
            <tr v-for="item in d5Logistics" :key="'d5l-' + item.value">
              <td>logistics</td><td>{{ item.value }}</td><td>{{ d5LogisticsT(item.value) }}</td>
            </tr>
            <tr v-for="item in d5Warehouse" :key="'d5w-' + item.value">
              <td>warehouse</td><td>{{ item.value }}</td><td>{{ d5WarehouseT(item.value) }}</td>
            </tr>
          </tbody>
        </table>
      </div>
...
const loading = computed(() =>
  ...
  l11.value || l12.value || l13.value || l14.value || l15.value,
);
</script>

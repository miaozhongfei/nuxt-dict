...
> `~/` نام مستعار مسیر Nuxt است که به ریشه سورس پروژه (srcDir) اشاره می‌کند. در Nuxt 4، srcDir پیش‌فرض `app/` است، بنابراین `~/dict/{storeName}-adapter.ts` معادل `app/dict/{storeName}-adapter.ts` روی دیسک است و پوشه `dict/` باید دستی ایجاد شود. به [آداپتور سفارشی](/advanced/custom-adapter) مراجعه کنید.

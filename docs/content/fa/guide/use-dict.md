...
`useDict` یک composable **در سطح کامپوننت** است. هر کامپوننت فراخوان `shallowRef` مستقل خود را دارد. داده‌ها در زمان mount (`onMounted`) به‌طور خودکار دریافت می‌شوند و با تغییر داده‌ها، تمپلیت به‌طور خودکار مجدداً رندر می‌شود. تغییر زبان باعث بارگذاری مجدد خودکار می‌شود.

- **واکنش‌گرا**: بله. `data` یک `ShallowRef` است — Vue تغییرات ارجاع را ردیابی می‌کند
- **محل فراخوانی**: باید در سطح بالای `<script setup>` فراخوانی شود
- **مناسب برای**: اتصال تمپلیت (گزینه‌های select، رندر لیست، نمایش درختی)

`$dict` یک مترجم همگام **سراسری** بدون واکنش‌گرایی Vue است. [مستندات $dict](/guide/dollar-dict) را ببینید.

...
`useDict` آرایه‌ای از `{ label, value }` برمی‌گرداند که مستقیماً با prop `items` در Nuxt UI سازگار است — بدون نیاز به تبدیل.

```vue
<template>
  <USelect v-model="value" :items="options" placeholder="جنسیت را انتخاب کنید" :loading="loading" />
</template>

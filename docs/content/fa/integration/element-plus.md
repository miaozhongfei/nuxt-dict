...
function getStatusColor(code: number): string | undefined {
  const item = statusData.value?.find(i => i.code === code)
...
<template>
  <el-cascader
    ...
    clearable
  />
  <p v-if="selected.length">مسیر کامل: {{ $dict.translatePath('region', selected[selected.length - 1]) }}</p>
</template>
...
function getColor(code: string | number): string | undefined {
  const item = data.value?.find(i => i.code === code)
  return (item as any)?.color
}
</script>
```
...
<template>
  <el-form :model="form" label-width="80px">
    ...
    <el-form-item label="منطقه">
      <el-cascader v-model="form.region" :options="regionTree || []" :props="{ value: 'code', label: 'label', children: 'children' }" />
    </el-form-item>
  </el-form>
</template>

<script setup lang="ts">
const { options: genderOptions, loading: genderLoading } = useDictOptions('gender')
const { options: industryOptions, loading: industryLoading } = useDictOptions('industry')

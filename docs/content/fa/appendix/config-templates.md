...
آداپتورها در فایل‌های مستقل با استفاده از `defineDictAdapter()` تعریف می‌شوند. فایل را در `~/dict/dict-adapter.ts` (مسیر قراردادی آداپتور سراسری) قرار دهید، ماژول آن را به صورت خودکار شناسایی می‌کند. `~/` نام مستعار srcDir در Nuxt است (در Nuxt 4، srcDir پیش‌فرض `app/` است، بنابراین مسیر واقعی `app/dict/dict-adapter.ts` است و پوشه `dict/` باید دستی ایجاد شود).

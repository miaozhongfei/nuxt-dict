...
| 类型                  | 默认值      | 说明                                                                                                                                                                                                                         |
| --------------------- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `string \| undefined` | `undefined` | 自定义适配器的文件路径（如 `'~/dict/dict-adapter.ts'`）。若文件存在于约定路径 `~/dict/dict-adapter.ts`，模块会自动发现并注册，无需手动配置。命名仓库使用 `stores.xxx.adapter`，详见 [自定义适配器](/advanced/custom-adapter) |
...
- **`adapter`**：该仓库的自定义适配器文件路径（`string`，**不继承**，未配则自动创建 REST 适配器）
- **`lazy`**：是否惰性检查版本号（`boolean`，未配继承全局 `api.lazy`，默认 `false`）

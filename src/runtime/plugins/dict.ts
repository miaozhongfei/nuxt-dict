 ...
 * 失败仅警告，不影响主流程。
 */
async function initClientCache(manager: DictManager, indexedDB: IndexedDBCache, logger: ReturnType<typeof createLogger>): Promise<void> {
  try {
 ...
 * 允许部分失败，不阻塞页面响应。
 */
async function executePrefetch(manager: DictManager, types: string[], logger: ReturnType<typeof createLogger>): Promise<void> {
  try {
...
export default defineNuxtPlugin(async (nuxtApp) => {
  ...
  const locale = import.meta.server
    ? resolveServerLocale(options)
    : resolveClientLocale(options)

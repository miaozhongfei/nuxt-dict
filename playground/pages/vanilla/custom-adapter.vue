<template>
  <div>
    ...
    <p style="color: #666">
      ...
      <code>/api/dict/list</code>；<code>dicts2</code>（配置了
      <code>dictEndpoint: '/api/dict/list2'</code>）调用不同端点。 打开
      <strong>DevTools Network 面板</strong> 可看到两个不同的请求。
    </p>
    ...
    <template v-else>
      ...
      <p style="color: #666">
        端点: <code>/api/dict/list</code> | adapter.storeName = <code>'dicts'</code>
      </p>
      ...
      <p style="color: #666">
        端点: <code>/api/dict/list2</code> | adapter.storeName = <code>'dicts2'</code>
      </p>
      <table border="1" cellpadding="6" cellspacing="0" style="border-collapse: collapse">
        <thead>
          <tr>
      ...
      <div style="margin-top: 12px">
        <button @click="doStore2Refresh" style="cursor: pointer">
          刷新 — 查看 /api/dict/list2 请求
        </button>

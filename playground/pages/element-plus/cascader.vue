<template>
  <div>
    ...
    <div v-else style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
      <span>选择地区：</span>
      <el-cascader v-model="value" :options="cascaderOptions" placeholder="请选择" style="width:300px;" clearable />
      <span v-if="value?.length">选中路径：<b>{{ value.join(' / ') }}</b></span>
    </div>
    <details style="margin-top:16px;"><summary>cascader options</summary><pre>{{ JSON.stringify(cascaderOptions, null, 2) }}</pre></details>
  </div>
</template>

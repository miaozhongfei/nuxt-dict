<template>
  <div>
    ...
    <div v-else style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
      ...
      <span>选中值：<code>{{ value }}</code></span>
      <span>翻译：<b>{{ value ? translate(value) : '-' }}</b></span>
    </div>
    <details style="margin-top:16px;"><summary>options 数据</summary><pre>{{ JSON.stringify(options, null, 2) }}</pre></details>
  </div>
</template>

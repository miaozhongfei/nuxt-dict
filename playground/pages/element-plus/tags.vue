  ...
  <div>
    ...
    <div v-else>
      <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:16px;">
        <el-tag v-for="item in data" :key="item.code" :color="(item as any).color" effect="dark" size="large">
          {{ item.label }}
        </el-tag>
      </div>
      ...
      <span v-if="selected" style="margin-left:8px;">
        <el-tag :color="(data?.find(i => i.code === selected) as any)?.color" effect="dark">
          {{ data?.find(i => i.code === selected)?.label }}
        </el-tag>
      </span>
    </div>
  </div>
